...

          {
            isOwnMessage && !message.isDeleted && (
              <div className="flex items-center -mr-1">
                {message.status === "sent" ? (
                  <BsCheck className="text-zinc-500 dark:text-zinc-400 text-[20px] stroke-[0.5]" />
                ) : (
                  <BsCheckAll className={`${message.status === "seen" ? "text-sky-500" : "text-zinc-500 dark:text-zinc-400"} text-[20px] stroke-[0.5]`} />
                )}
              </div>
            )
          }
